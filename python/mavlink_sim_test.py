...
if __name__ == "__main__":
    ...
    # ---------- Send initial system_time message ----------

    #absolute time in micro seconds
    time_unix_us = round(time.time() * 1e6)
    print("current time us:: ", time_unix_us)

    #time since boot
    time_since_boot_us = time_unix_us - time_boot_us
    print("Time since boot us:: ", time_since_boot_us)
    time_since_boot_ms = round(time_since_boot_us / 1000)
    print("Time since boot ms:: ", time_since_boot_ms)
    ...
    # ---------- Send initial hil_sensor message ----------
    update_hil(
        time_since_boot_us,
        x_accel,
        y_accel,
    ...
    # ---------- Begin hil_sensor and hil_state loop ----------
    while True:
        # Update time
        time_unix_us = round(time.time() * 1e6)
        time_since_boot_us = time_unix_us - time_boot_us
